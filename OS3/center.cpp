...
int main()
{
    ...
    if (server->Bind(address) != NO_ERROR) { return -1; }
...
{
    counter_mutex.lock(); uint16_t temp = counter.load();
    if (temp != 65535)counter++; counter_mutex.unlock(); return temp;
}

void machineHandler(const TCPSocketPtr &client)
{
    char buffer[3]; buffer[0] = 'y';
    auto temp = getAddCounter(); memcpy(buffer + 1, (char *) &temp, 2);
    while (client->Send(buffer, 3))
    {
        char rec_buffer[4]; client->Recieve(rec_buffer, sizeof(float));
        ...
        temp = getAddCounter();  memcpy(buffer+1,(char *) &temp, 2);
    }
}
...
int writeFile(uint16_t index, float result)
{
    ...
    if (file.is_open()){ file << index << " " << result << "\n"; } else std::cout << "FILE PROBLEMS";
    file.close(); curr_counter++; return 1;
}
